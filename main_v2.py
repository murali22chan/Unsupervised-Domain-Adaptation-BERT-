...
class BertDomainAdaptation(nn.Module):
    ...
    def forward(self, source_input_ids, source_attention_mask=None, target_input_ids=None, target_attention_mask=None,
                human_machine_labels=None):
        ...
        if target_input_ids is not None:
            ...
            return human_machine_loss + self.beta * loss, loss, human_machine_logits, src_logits, src_labels, tgt_logits, tgt_labels
...
def split_data_training_validation_testing(train_df, test_df, split_ratio=0.4):
    train_text, val_text, train_labels, val_labels = train_test_split(train_df['answer'], train_df['label'],
                                                                      random_state=0, test_size=split_ratio,
...
# Function to create a dataset loader
def create_dataset_loader_DA(train_text, train_labels, val_text, val_labels, train_text_cd, train_labels_cd,
                             train_sequence_length, test_sequence_length, batch_size=6):
    tokens_train = tokenizer.batch_encode_plus(
        train_text.tolist(),
        max_length=train_sequence_length,
        pad_to_max_length=True,
...
def evaluate(val_dataloader):
    ...
    for step, batch in enumerate(val_dataloader):
        ...
        with torch.no_grad():
            # model predictions
            loss, domain_loss, preds, _, _, src_logits, src_labels = model(source_input_ids=sent_id, source_attention_mask=mask,
                                                         target_input_ids=sent_id,
                                                         target_attention_mask=mask, human_machine_labels=labels)
            ...
            _, src_predicted = torch.max(src_logits, 1)
            src_acc.extend(src_predicted.tolist())
            src_labels_f.extend(src_labels.tolist())

    # compute the validation loss of the epoch

    avg_loss = total_loss / len(val_dataloader)
    avg_domain_loss = total_domain_loss / len(val_dataloader)
    src_accuracy = accuracy_score(src_labels_f, src_acc)

    # reshape the predictions in form of (number of samples, no. of classes)

    total_preds = np.concatenate(total_preds, axis=0)

    return avg_loss, total_preds, avg_domain_loss, src_accuracy
...
def test(val_dataloader):
    ...
    for step, batch in enumerate(val_dataloader):
        ...
        with torch.no_grad():
            # model predictions
            loss, domain_loss, preds, _, _, tgt_logits, tgt_labels = model(source_input_ids=sent_id, source_attention_mask=mask,
                                             target_input_ids=mask,
                                             target_attention_mask=mask, human_machine_labels=labels)
            ...
            total_preds.append(preds)
            tgt_logits = F.softmax(tgt_logits, dim=1)
            _, tgt_predictied = torch.max(tgt_logits, 1)
            tgt_acc.extend(tgt_predictied.tolist())
            tgt_labels_f.extend(tgt_labels.tolist())

    # compute the validation loss of the epoch

    avg_loss = total_loss / len(val_dataloader)
    avg_domain_loss = total_domain_loss / len(val_dataloader)
    tgt_accuracy = accuracy_score(tgt_labels_f, tgt_acc)

    # reshape the predictions in form of (number of samples, no. of classes)

    total_preds = np.concatenate(total_preds, axis=0)

    return avg_loss, total_preds, avg_domain_loss, tgt_accuracy


# Function to get training accuracy
def get_train_accuracy(lager_data_loader, small_data_loader):
    model.eval()
    src_acc = []
    tgt_acc = []
    src_labels_f = []
    ...
    for step, batch in tqdm.tqdm(enumerate(lager_data_loader)):
        ...
        with torch.no_grad():
            loss, domain_loss, preds_logits, src_logits, src_labels, tgt_logits, tgt_labels = model(source_input_ids=sent_id, source_attention_mask=mask,
                                                target_input_ids=sent_id_tgt, target_attention_mask=mask_tgt,
                                                human_machine_labels=labels)
    ...
    src_accuracy = accuracy_score(src_labels_f, src_acc)
    tgt_accuracy = accuracy_score(tgt_labels_f, tgt_acc)
    src_labels_f.extend(tgt_labels_f)
    src_acc.extend(tgt_acc)
    final_accuracy = accuracy_score(src_labels_f, src_acc)

    return src_accuracy, tgt_accuracy, final_accuracy
...
if __name__ == '__main__':
    ...
    print("Training Data: " + str(args.trainDomain) + " Testing Data: " + str(
        args.testDomain) + " Training Sequnce Length: " + str(args.trainSeqLength) + " Testing Sequence Length: " + str(
        args.testSeqLength))
    ...
    # train_text, train_labels, val_text, val_labels, test_text, test_labels = split_data_training_validation_testing_same_domain(train_df)
        ...
        train_text, train_labels, val_text, val_labels, _, _ = split_data_training_validation_testing_same_domain(
            train_df)
        train_text_cd, train_labels_cd, val_text_cd, val_labels_cd, test_text, test_labels = split_data_training_validation_testing_same_domain(
            cross_domain_df)
    ...
    _, _, test_dataloader = create_dataset_loader(train_text, train_labels, val_text, val_labels, test_text,
                                                  test_labels, trainSequenceLength, testSequenceLength, batch_size)
    train_dataloader, val_dataloader, train_dataloader_cd = create_dataset_loader_DA(train_text, train_labels, val_text,
                                                                                     val_labels, train_text_cd,
                                                                                     train_labels_cd,
                                                                                     trainSequenceLength,
                                                                                     testSequenceLength, batch_size)
    ...
    model = BertDomainAdaptation(num_labels=2, alpha=alpha, beta=beta)
    model.cuda()
    optimizer = torch.optim.Adam(model.parameters(), lr=lr_classification)

    # optimizer_domain = torch.optim.Adam(model.classifier.parameters(), lr=lr_domain)

    config = wandb.config
    config.learning_rate = lr_classification
    # config.learning_rate_domain = lr_domain
    config.num_epochs = epochs
    config.batch_size = batch_size

    wandb.run.name = "bert_train_" + str(args.trainDomain) + "_test_" + str(args.testDomain) + "_trainSeqLen_" + str(
    ...
    for epoch in tqdm.tqdm(range(epochs)):
        ...
        for step, batch in tqdm.tqdm(enumerate(lager_data_loader)):
            ...
            loss, domain_loss, preds_logits, _, _, _, _  = model(source_input_ids=sent_id, source_attention_mask=mask,
                                                    target_input_ids=sent_id_tgt, target_attention_mask=mask_tgt,
                                                    human_machine_labels=labels)
        ...
        valid_loss, val_preds, val_domain_loss, val_domain_acc = evaluate(val_dataloader)
        ...
        src_accuracy, tgt_accuracy, final_domain_train_accuracy = get_train_accuracy(lager_data_loader, small_data_loader)

        # save the best model
        if valid_accuracy > best_valid_acc:
            best_valid_acc = valid_accuracy
            model_name = "bert_train_" + str(args.trainDomain) + "_test_" + str(
        ...
        train_total_loss = train_total_loss / domain_1_samples
        train_domain_loss = train_domain_loss / domain_1_samples
        wandb.log({"epoch": epoch + 1, "training_loss": train_total_loss, "validation_loss": valid_loss,
                   "validation_accuracy": valid_accuracy, 'training_domain_loss': train_domain_loss,
                   'validation_domain_loss': val_domain_loss, "training_source_accuracy": src_accuracy,
                   "training_target_accuracy": tgt_accuracy, "training_domain_accuracy": final_domain_train_accuracy, "alpha":alpha, "beta":beta})

    average_loss, total_preds, test_domain_loss, test_domain_accuracy = test(test_dataloader)  # Testing
    ...
    labels = [0, 1]
    cm = confusion_matrix(test_y, preds, labels=labels)


    # Logging to Wandb
    wandb.log({"accuracy": accuracy, "precision": macro_precision, "recall": macro_recall, "f1-score": macro_f1,
               "testing_domain_loss": test_domain_loss, "test_domain_accuracy": test_domain_accuracy})
    ...
    results = {"training_data": args.trainDomain, "testing_data": args.testDomain,
               "training_seq_len": args.trainSeqLength, "testing_seq_len": args.testSeqLength, "accuracy": accuracy,
               "precision": macro_precision, "recall": macro_recall, "f1-score": macro_f1}
